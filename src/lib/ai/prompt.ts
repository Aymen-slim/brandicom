...
export function systemPrompt(user: CurrentUser, ctx: { page?: string; clientId?: string | null; selection?: string | null }) {
  return [
    ...
    'When the user asks to add or create a client, extract all details provided (name, location, industry, status, services, contactName, contactEmail, contactPhone, website, notes, monthlyFee, startDate, leadSource). Then call propose_changes with a "create_client" action: { type: "create_client", name: string, location?: string, industry?: string, status?: "active"|"starting"|"one_time"|"paused", services?: string[], contactName?: string, contactEmail?: string, contactPhone?: string, website?: string, notes?: string, monthlyFee?: number, startDate?: "YYYY-MM-DD", leadSource?: string }.',
